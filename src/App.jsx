...
const COLORS = {
  cyan: { text: "text-cyan-400", hoverBorder: "hover:border-cyan-500", groupHoverShadow: "group-hover:shadow-cyan-500/30" },
  purple: { text: "text-purple-400", hoverBorder: "hover:border-purple-500", groupHoverShadow: "group-hover:shadow-purple-500/30" }
};
...
const SelectionCard = ({ label, make, model, img, set, colorTheme, isTexture, isRim, isProfiling, onRemove, showRemove }) => {
  ...
  const renderHeader = () => typeof label === 'object' ? (
    <div className="flex flex-col items-center justify-center w-full text-center gap-0.5 py-1">
      <p className="text-[12px] font-black uppercase text-slate-500 tracking-wider whitespace-normal leading-tight px-2">Name={label.name}</p>
      <p className="text-[11px] font-black uppercase text-slate-600 tracking-wider whitespace-normal leading-tight px-2">Short-name={label.shortName}</p>
    </div>
  ) : <span className="text-[28px] font-black uppercase text-slate-500 tracking-[0.15em] shrink-0 text-center leading-none py-1 drop-shadow-lg">{label}</span>;
  ...
  return (
    <div className={`bg-[#151B28] ${isTexture ? 'p-5' : 'p-8 pt-6'} rounded-[3.5rem] border border-slate-800/80 flex flex-col items-center shadow-2xl space-y-5 w-full h-full transition-all relative overflow-hidden`}>
      ...
      {isTexture && showRemove && <button onClick={(e) => { e.stopPropagation(); onRemove(); }} className="absolute top-4 right-4 p-3 bg-red-600/10 text-red-500 rounded-2xl border border-red-600/20 hover:bg-red-600 hover:text-white transition-all z-10"><Trash2 size={18} /></button>}
  ...
  const handleDrop = async (e) => {
    ...
    if (tyresIniFile) {
      ...
      if (uniqueCompounds.length > 0) {
        const newTexList = uniqueCompounds.map(label => ({ make: 'Valino', model: 'Pergea 08R', label }));
    ...
    <div className="min-h-screen bg-[#0B0F19] text-white font-sans px-6 md:px-12 selection:bg-cyan-500/30 overflow-x-hidden pb-24 relative">
      ...
      {genStep === 'generating' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-2xl">
          <div className="w-full max-w-lg bg-[#151B28] border border-slate-800 rounded-[3.5rem] shadow-2xl p-14 text-center">
            ...
            <h3 className="text-2xl font-black tracking-tight mb-2 uppercase italic text-white">Generating pack...</h3>
          </div>
        </div>
      )}
      ...
      {showGate && (
        <div className="fixed inset-0 z-[101] flex items-center justify-center p-4 bg-black/95 backdrop-blur-2xl">
          <div className="w-full max-w-md bg-[#151B28] border border-slate-800 rounded-[3.5rem] shadow-2xl p-12 text-center">
            ...
            <p className="text-slate-400 text-sm my-6">You've used all 5 free generations.<br/>Subscribe to the $10 tier for unlimited access.</p>
            <a href="https://www.patreon.com/membership/26118508" target="_blank" className="block w-full py-6 bg-[#FF424D] hover:bg-[#E33B44] text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl">Subscribe Now – $10/month</a>
            <button onClick={() => setShowGate(false)} className="mt-6 text-slate-400 hover:text-white text-sm underline">Close</button>
          </div>
        </div>
      )}

      <div className="max-w-[1900px] mx-auto pt-10 pb-12">
        <header className="flex flex-col items-center justify-center space-y-4 pb-10">
          <img src={formatImageUrl(`id=${LOGO_ID}`)} alt="Logo" className="h-40 drop-shadow-[0_0_35px_rgba(34,211,238,0.6)]" />
        ...
        <div className="flex flex-col gap-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 xl:gap-12">
            <div className="flex flex-col gap-6">
              <SelectionCard label="Front Rim" make={frontMake} model={frontModel} img={currentFrontRim.img} set={() => setPickerOpen('front-rim')} colorTheme="cyan" isRim />
